...
public class EbonoakGen extends AbstractTreeFeature<NoFeatureConfig> {
   private static final BlockState LOG = BlockRegistry.EBONOAK_LOG.getDefaultState();
   private static final BlockState LEAVES = BlockRegistry.EBONOAK_LEAVES.getDefaultState();

   public EbonoakGen(Function<Dynamic<?>, ? extends NoFeatureConfig> p_i49895_1_, boolean p_i49895_2_) {
      super(p_i49895_1_, p_i49895_2_);
      setSapling((net.minecraftforge.common.IPlantable)BlockRegistry.EBONOAK_SAPLING);
   }

   public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldIn, Random rand, BlockPos position, MutableBoundingBox p_208519_5_) {
      int i = rand.nextInt(3) + rand.nextInt(2) + 8;
      int j = position.getX();
      int k = position.getY();
      int l = position.getZ();
      if (k >= 1 && k + i + 1 < 256) {
