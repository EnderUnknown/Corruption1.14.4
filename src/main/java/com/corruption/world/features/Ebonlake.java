...
public class Ebonlake extends Feature<LakesConfig> {
   private static final BlockState AIR = Blocks.CAVE_AIR.getDefaultState();
   private BlockState surroundingBlock = Blocks.GRASS_BLOCK.getDefaultState();

   public Ebonlake(Function<Dynamic<?>, ? extends LakesConfig> lakeConfig, BlockState surroundingBlock) {
      super(lakeConfig);
                  ...
                  for(int j4 = 4; j4 < 8; ++j4) {
                     if (aboolean[(i2 * 16 + j3) * 8 + j4]) {
                        ...
                        if (Block.isDirt(worldIn.getBlockState(blockpos).getBlock()) && worldIn.getLightFor(LightType.SKY, pos.add(i2, j4, j3)) > 0) {
                           worldIn.setBlockState(blockpos, surroundingBlock, 2);
                        }
                     }
                  }
               }
