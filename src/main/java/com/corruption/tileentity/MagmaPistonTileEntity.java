...
public class MagmaPistonTileEntity extends TileEntity implements ITickableTileEntity {
   ...
   public MagmaPistonTileEntity() {
      super(BlockRegistry.MAGMAPISTON_TE);
   }

   public MagmaPistonTileEntity(BlockState pistonStateIn, Direction pistonFacingIn, boolean extendingIn, boolean shouldHeadBeRenderedIn) {
      this();
   ...
   private BlockState getCollisionRelatedBlockState() {
      return !this.isExtending() && this.shouldPistonHeadBeRendered() && this.pistonState.getBlock() instanceof MagmaPiston ? BlockRegistry.MAGMAPISTON_HEAD.getDefaultState().with(MagmaPistonHeadBlock.TYPE, this.pistonState.getBlock() == BlockRegistry.MAGMASTICKY_PISTON ? PistonType.STICKY : PistonType.DEFAULT).with(MagmaPistonHeadBlock.FACING, this.pistonState.get(MagmaPiston.FACING)) : this.pistonState;
   }
   ...
   public void clearPistonTileEntity() {
      if (this.lastProgress < 1.0F && this.world != null) {
         ...
         this.remove();
         if (this.world.getBlockState(this.pos).getBlock() == BlockRegistry.MAGMAMOVINGPISTON) {
   ...
   public void tick() {
      ...
      if (this.lastProgress >= 1.0F) {
         this.world.removeTileEntity(this.pos);
         this.remove();
         if (this.pistonState != null && this.world.getBlockState(this.pos).getBlock() == BlockRegistry.MAGMAMOVINGPISTON) {
   ...
   public VoxelShape getCollisionShape(IBlockReader p_195508_1_, BlockPos p_195508_2_) {
      ...
      if (!this.extending && this.shouldHeadBeRendered) {
         voxelshape = this.pistonState.with(MagmaPiston.EXTENDED, Boolean.valueOf(true)).getCollisionShape(p_195508_1_, p_195508_2_);
      } else {
      ...
      if ((double)this.progress < 1.0D && direction == this.getMotionDirection()) {
         ...
         if (this.shouldPistonHeadBeRendered()) {
            blockstate = BlockRegistry.MAGMAPISTON_HEAD.getDefaultState().with(MagmaPistonHeadBlock.FACING, this.pistonFacing).with(MagmaPistonHeadBlock.SHORT, Boolean.valueOf(this.extending != 1.0F - this.progress < 4.0F));
         } else {
