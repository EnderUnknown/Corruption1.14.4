...
public class MagmaPistonBlockStructureHelper {
   ...
   public MagmaPistonBlockStructureHelper(World worldIn, BlockPos posIn, Direction pistonFacing, boolean extending) {
      this.world = worldIn;
   ...
   public boolean canMove() {
      ...
      if (!MagmaPiston.canPush(blockstate, this.world, this.blockToMove, this.moveDirection, false, this.facing)) {
   ...
   private boolean addBlockLine(BlockPos origin, Direction facingIn) {
      ...
      if (world.isAirBlock(origin)) {
         return true;
      } else if (!MagmaPiston.canPush(blockstate, this.world, origin, this.moveDirection, false, facingIn)) {
         return true;
         ...
         int i = 1;
         if (i + this.toMove.size() > 100) {
            return false;
         } else {
            while(blockstate.isStickyBlock()) {
               BlockPos blockpos = origin.offset(this.moveDirection.getOpposite(), i);
               blockstate = this.world.getBlockState(blockpos);
               if (blockstate.isAir(this.world, blockpos) || !MagmaPiston.canPush(blockstate, this.world, blockpos, this.moveDirection, false, this.moveDirection.getOpposite()) || blockpos.equals(this.pistonPos)) {
                  break;
               }
            ...
            while(true) {
               ...
               if (!MagmaPiston.canPush(blockstate, this.world, blockpos1, this.moveDirection, true, this.moveDirection) || blockpos1.equals(this.pistonPos)) {
                  return false;
               }
