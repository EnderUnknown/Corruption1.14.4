...
import com.corruption.util.MagmaPistonBlockStructureHelper;
...
public class MagmaPiston extends DirectionalBlock {
   ...
   public MagmaPiston(boolean sticky, Block.Properties properties) {
      super(properties);
      ...
      //this.setRegistryName("magmapiston");
   }
   ...
   private void checkForMove(World worldIn, BlockPos pos, BlockState state) {
      ...
      if (flag && !state.get(EXTENDED)) {
         if ((new MagmaPistonBlockStructureHelper(worldIn, pos, direction, true)).canMove()) {
         ...
         int i = 1;
         if (blockstate.getBlock() == BlockRegistry.MAGMAMOVINGPISTON && blockstate.get(FACING) == direction) {
            TileEntity tileentity = worldIn.getTileEntity(blockpos);
            if (tileentity instanceof MagmaPistonTileEntity) {
            	MagmaPistonTileEntity pistontileentity = (MagmaPistonTileEntity)tileentity;
   ...
   public boolean eventReceived(BlockState state, World worldIn, BlockPos pos, int id, int param) {
      ...
      if (id == 0) {
         ...
         worldIn.playSound((PlayerEntity)null, pos, SoundRegistry.BLOCK_MAGMAPISTON_EXTEND, SoundCategory.BLOCKS, 0.5F, worldIn.rand.nextFloat() * 0.25F + 0.6F);
      } else if (id == 1 || id == 2) {
         TileEntity tileentity1 = worldIn.getTileEntity(pos.offset(direction));
         if (tileentity1 instanceof MagmaPistonTileEntity) {
            ((MagmaPistonTileEntity)tileentity1).clearPistonTileEntity();
         }

         worldIn.setBlockState(pos, BlockRegistry.MAGMAMOVINGPISTON.getDefaultState().with(MagmaMovingPiston.FACING, direction).with(MagmaMovingPiston.TYPE, this.isSticky ? PistonType.STICKY : PistonType.DEFAULT), 3);
         worldIn.setTileEntity(pos, MagmaMovingPiston.createTilePiston(this.getDefaultState().with(FACING, Direction.byIndex(param & 7)), direction, false, true));
         if (this.isSticky) {
            BlockPos blockpos = pos.add(direction.getXOffset() * 2, direction.getYOffset() * 2, direction.getZOffset() * 2);
            BlockState blockstate = worldIn.getBlockState(blockpos);
            Block block = blockstate.getBlock();
            boolean flag1 = false;
            if (block == BlockRegistry.MAGMAMOVINGPISTON) {
               TileEntity tileentity = worldIn.getTileEntity(blockpos);
               if (tileentity instanceof MagmaPistonTileEntity) {
            	   MagmaPistonTileEntity pistontileentity = (MagmaPistonTileEntity)tileentity;
            ...
            if (!flag1) {
               if (id != 1 || blockstate.isAir(worldIn, blockpos) || !canPush(blockstate, worldIn, blockpos, direction.getOpposite(), false, direction) || blockstate.getPushReaction() != PushReaction.NORMAL && block != Blocks.PISTON && block != Blocks.STICKY_PISTON && block != BlockRegistry.MAGMAPISTON&& block != BlockRegistry.MAGMASTICKY_PISTON) {
         ...
         worldIn.playSound((PlayerEntity)null, pos, SoundRegistry.BLOCK_MAGMAPISTON_RETRACT, SoundCategory.BLOCKS, 0.5F, worldIn.rand.nextFloat() * 0.15F + 0.6F);
      }
   ...
   public static boolean canPush(BlockState blockStateIn, World worldIn, BlockPos pos, Direction facing, boolean destroyBlocks, Direction p_185646_5_) {
      ...
      if (!worldIn.getWorldBorder().contains(pos)) {
         ...
         if (pos.getY() <= worldIn.getHeight() - 1 && (facing != Direction.UP || pos.getY() != worldIn.getHeight() - 1)) {
            if (block != BlockRegistry.MAGMAPISTON && block != BlockRegistry.MAGMASTICKY_PISTON && block!=Blocks.PISTON&&block!=Blocks.STICKY_PISTON) {
   ...
   private boolean doMove(World worldIn, BlockPos pos, Direction directionIn, boolean extending) {
      ...
      if (!extending && worldIn.getBlockState(blockpos).getBlock() == BlockRegistry.MAGMAPISTON_HEAD) {
         worldIn.setBlockState(blockpos, Blocks.AIR.getDefaultState(), 20);
      }

      MagmaPistonBlockStructureHelper pistonblockstructurehelper = new MagmaPistonBlockStructureHelper(worldIn, pos, directionIn, extending);
      if (!pistonblockstructurehelper.canMove()) {
         ...
         for(int l = list.size() - 1; l >= 0; --l) {
            ...
            worldIn.setBlockState(blockpos3, BlockRegistry.MAGMAMOVINGPISTON.getDefaultState().with(FACING, directionIn), 68);
            worldIn.setTileEntity(blockpos3, MagmaMovingPiston.createTilePiston(list1.get(l), directionIn, extending, false));
            --k;
         ...
         if (extending) {
            ...
            BlockState blockstate1 = BlockRegistry.MAGMAPISTON_HEAD.getDefaultState().with(MagmaPistonHeadBlock.FACING, directionIn).with(MagmaPistonHeadBlock.TYPE, pistontype);
            BlockState blockstate4 = BlockRegistry.MAGMAMOVINGPISTON.getDefaultState().with(MagmaMovingPiston.FACING, directionIn).with(MagmaMovingPiston.TYPE, this.isSticky ? PistonType.STICKY : PistonType.DEFAULT);
            set.remove(blockpos);
            worldIn.setBlockState(blockpos, blockstate4, 68);
            worldIn.setTileEntity(blockpos, MagmaMovingPiston.createTilePiston(blockstate1, directionIn, true, true));
         }
         ...
         if (extending) {
            worldIn.notifyNeighborsOfStateChange(blockpos, BlockRegistry.MAGMAPISTON_HEAD);
         }
